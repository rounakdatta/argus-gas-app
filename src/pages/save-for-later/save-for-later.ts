    ...
    this.sqlite.create({
      ...
      .then(res => {
        ...
        for(var i=0; i<res.rows.length; i++) {
            let temp_unit = CANADA_TEMP_UNIT;
            let test_weight_unit = CANADA_TEST_WEIGHT_UNIT;
            if(res.rows.item(i).countryId == '1')
            {temp_unit = US_TEMP_UNIT;
             test_weight_unit = US_TEST_WEIGHT_UNIT;
             }
          ...
          this.savedArr.push({
            ...
            countryId: res.rows.item(i).countryId,
            temp_unit: temp_unit,
